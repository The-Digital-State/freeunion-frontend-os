import { basicNavigator, buildUIExtensionRegistry, Config, extendNavigator } from '@owlmeans/regov-lib-react/dist/common';
import { WalletHandler } from '@owlmeans/regov-ssi-core';
import { useHistory } from 'react-router-dom';
import { authIntegratedExtension } from '@owlmeans/regov-ext-auth';
import { buildIdentityExtensionUI } from '@owlmeans/regov-ext-identity';
import { groupsUIExtension } from '../ext-groups';
...
export const regovConfig = {
  ...
  baseSchemaUrl: process.env.REACT_APP_BASE_SCHEMA_URL || 'https://api.freeunion.online/schemas',
...
//       timeout: process.env.REACT_APP_DIDCOMM_TIMEOUT
//         ? parseInt(process.env.REACT_APP_DIDCOMM_TIMEOUT) : 30
//     }
//   }
// };
...
export const regovNavigatorBuilder = (handler: WalletHandler, config: Config, history: ReturnType<typeof useHistory>) =>
  extendNavigator(basicNavigator, {
    ...
    home: async () => {
      // setTimeout(() => history.push(routes.DASHBOARD), 100);
    },
...
registry.registerSync(
  buildIdentityExtensionUI(
    ...
    { appName: regovConfig.name },
    {
      ...
      schemaBaseUrl: process.env.REACT_APP_BASE_SCHEMA_URL || 'https://api.freeunion.online/schemas',
    }
  )
);
