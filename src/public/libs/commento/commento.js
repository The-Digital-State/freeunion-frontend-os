...
(function (global, document) {
  ...
  function selfLoad(commenter, email) {
    ...
    notificationSettingsButton.innerText = "Настройки уведомлений";
    profileEditButton.innerText = "Редактировать профиль";
  ...
  function markdownHelpShow(id) {
    ...
    boldLeft.innerHTML = "<b>жирный</b>";
    boldRight.innerHTML = "возьмите текст в <pre>**двойные звёздочки**</pre>";
    italicsLeft.innerHTML = "<i>курсив</i>";
    italicsRight.innerHTML = "возьмите текст в <pre>*звёздочки*</pre>";
    codeLeft.innerHTML = "<pre>код</pre>";
    codeRight.innerHTML = "возьмите текст в <pre>`обратные кавычки`</pre>";
    strikethroughLeft.innerHTML = "<strike>зачёркнутый</strike>";
    strikethroughRight.innerHTML = "возьмите текст в <pre>~~двойные тильды~~</pre>";
    hyperlinkLeft.innerHTML = "<a href=\"https://example.com\">ссылка</a>";
    hyperlinkRight.innerHTML = "<pre>[ссылка](https://example.com)</pre> или просто вставьте адрес";
    quoteLeft.innerHTML = "<blockquote>цитата</blockquote>";
    quoteRight.innerHTML = "начните текст со знака \"больше\" <pre>&gt;</pre>";
  ...
  function textareaCreate(id, edit) {
    ...
    attrSet(textarea, "placeholder", "Добавить комментарий");
    attrSet(anonymousCheckbox, "type", "checkbox");
    attrSet(anonymousCheckboxLabel, "for", ID_ANONYMOUS_CHECKBOX + id);

    anonymousCheckboxLabel.innerText = "Анонимно";
    if (edit === true) {
      submitButton.innerText = "Сохранить";
    } else {
      submitButton.innerText = "Добавить комментарий";
    }
    markdownButton.innerHTML = "<b>M &#8595;</b> &nbsp; Разметка";
  ...
  var sortPolicyNames = {
    "score-desc": "Голоса",
    "creationdate-desc": "Новые",
    "creationdate-asc": "Старые",
  };
  ...
  function rootCreate(callback) {
    ...
    if (loginAvailable) {
      loginText.innerText = "Войти под псевдонимом";
      onclick(loginText, global.loginBoxShow, null);
    } else {
      loginText.innerHTML = "Для входа под псевдонимом <a href='/login'>авторизуйтесь</a> на сайте";
    }
    ...
    if (isLocked || isFrozen) {
      if (isAuthenticated || chosenAnonymous) {
        append(mainArea, messageCreate("Это обсуждение закрыто."));
        remove($(ID_LOGIN));
      } else {
  ...
  global.commentNew = function (id, commenterToken, callback) {
    ...
    post(origin + "/api/comment/new", json, function (resp) {
      ...
      if (root && root.onCommentNew) {
        root.onCommentNew(pageId);
      }
      ...
      if (resp.state === "unapproved") {
        ...
        message = "Ваш комментарий отмечен как спам и ожидает проверки.";
      }
      ...
      if (id !== "root") {
        ...
        replyButton.title = "Ответить на этот комментарий";
  ...
  function commentsRecurse(parentMap, parentHex) {
    ...
    cur.forEach(function (comment) {
      ...
      collapse.title = "Закрыть дочерние элементы";
      upvote.title = "Голосовать ЗА";
      downvote.title = "Голосовать ПРОТИВ";
      edit.title = "Редактировать";
      reply.title = "Ответить";
      approve.title = "Одобрить";
      remove.title = "Удалить";
      if (stickyCommentHex === comment.commentHex) {
        if (isModerator) {
          sticky.title = "Открепить";
        } else {
          sticky.title = "Комментарий закреплён";
        }
      } else {
        sticky.title = "Sticky";
      }
  ...
  function commentEdit(id) {
    ...
    post(origin + "/api/comment/edit", json, function (resp) {
      ...
      if (root && root.onCommentEdit) {
        root.onCommentEdit(pageId);
      }
      ...
      var message = "";
      if (resp.state === "unapproved") {
        message = "Ваш комментарий находится на модерации.";
      } else if (resp.state === "flagged") {
        message = "Ваш комментарий отмечен как спам и находится на модерации.";
      }
  ...
  global.commentCollapse = function (id) {
    ...
    button.title = "Показать дочерние элементы";
  ...
  global.commentUncollapse = function (id) {
    ...
    button.title = "Спрятать дочерние элементы";
  ...
  global.popupRender = function (id) {
    ...
    forgotLink.innerText = "Забыли пароль?";
    loginLink.innerText = "Не имеете учетной записи? Зарегистрируйтесь.";
    emailSubtitle.innerText = "Войти с помощью адреса электронной почты.";
    emailButton.innerText = "Продолжить";
    oauthSubtitle.innerText = "Войти через социальные сети.";
    ssoSubtitle.innerText = "Войти под псевдонимом";
    ...
    attrSet(emailInput, "placeholder", "Адрес электронной почты");
  ...
  global.popupSwitch = function (id) {
    ...
    emailSubtitle.innerText = "Создать учетную запись";
    popupBoxType = "signup";
  ...
  global.passwordAsk = function (id) {
    ...
    if (popupBoxType === "signup") {
      ...
      placeholder = ["Имя", "Сайт (Необязательно)", "Пароль"];
    } else {
      order = ["password"];
    ...
    if (popupBoxType === "signup") {
      subtitle.innerText = "Для завершения регистрации заполните профиль до конца."
    } else {
      subtitle.innerText = "Введите пароль чтобы войти."
    }
  ...
  function modToolsCreate() {
    ...
    if (isLocked) {
      lock.innerHTML = "Разблокировать обсуждение";
    } else {
      lock.innerHTML = "Заблокировать обсуждение";
    }
  ...
  function dataTagsLoad() {
    ...
    for (var i = 0; i < scripts.length; i++) {
      if (scripts[i].src.match(/\/commento\/commento\.js$/)) {
        ...
        loginAvailable = attrGet(scripts[i], "data-login-available") === "true";
      }
    }
  }
      ...
      if (window.location.hash.startsWith("#commento-")) {
        ...
        if (el === null) {
          if (id.length === 64) {
            ...
            errorShow("Комментарий который ищете не существует либо был удалён.");
          }
          return;
        }
